...
const APT_COMPLEXES = [
  { name: "반도유보라 퍼스티지 아파트", dongStart: 101, dongEnd: 106 },
  { name: "제일풍경채 디오션", dongStart: 101, dongEnd: 107 },
...
const [addressComplex, setAddressComplex] = useState("");
...
const selectedComplex = APT_COMPLEXES.find(c => c.name === addressComplex);
const selectedComplexDongs = selectedComplex
  ? Array.from({ length: selectedComplex.dongEnd - selectedComplex.dongStart + 1 }, (_, i) => `${selectedComplex.dongStart + i}동`)
  : [];
...
const address = `${addressComplex} ${addressDong} ${addressHo.trim()}`.trim();
...
if (!addressComplex || !addressDong || !addressHo.trim()) errors.address = "아파트·동·호수를 입력해 주세요.";
...
<div><p><span className="hero-lead">단순청소, 깨끗함을 넘어,</span><span className="hero-lead hero-lead-accent">공간의 수명과 가치를 복원하는 <b className="apt-value-strong">프리미엄 홈 케어 솔루션</b>.</span></p></div>
</div>
</div>
...
adminAddOpenFor === key && React.createElement("div", { className: "admin-add-booking-form" },
React.createElement("label", null, "날짜",
React.createElement("input", { type: "date", value: adminAddDate, onChange: (e: any) => setAdminAddDate(e.target.value) })
),
...
timeSlots.map(([time, label]) => React.createElement("option", { key: time, value: time }, label))
)
),
...
{selectedDate && adminBookings.length === 0 ? <div className="admin-empty">아직 예약이 없습니다.</div> : selectedDate && adminBookings.map(booking => { const status = bookingStatuses[booking.id] ?? booking.booking_status ?? "예약접수"; return <article className="admin-day-booking-card" key={booking.id}><div className="admin-day-booking-info"><strong>{booking.name}</strong><span>{booking.phone}</span><span>{(booking.address || "주소 미입력").replace(APT_COMPLEX_PREFIX_RE, "")}</span><span>{booking.booking_time.slice(0, 5)}</span></div><div className="admin-status-buttons" aria-label={`${booking.name} 예약 상태`}>{(["예약접수", "통화필요"] as const).map(item => React.createElement("button", { type: "button", className: status === item ? "selected" : "", key: item, onClick: async () => {
await setBookingStatus(booking.id, item);
if (item === "통화필요") window.open(`tel:${booking.phone.replace(/\D/g, "")}`, "_blank");
} }, item === "통화필요" ? "통화" : item))}{React.createElement("button", { type: "button", onClick: () => window.open(`sms:${booking.phone.replace(/\D/g, "")}`, "_blank") }, "문자")}{(["예약확정", "예약취소"] as const).map(item => React.createElement("button", { type: "button", className: status === item ? "selected" : "", key: item, onClick: async () => {
if (item === "예약취소" && !window.confirm("정말 이 고객의 예약을 취소하시겠습니까?")) return;
await setBookingStatus(booking.id, item);
} }, item))
}<button type="button" disabled={booking.completed} onClick={() => completeBooking(booking.id)}>{booking.completed ? "서비스 완료 ✓" : "서비스 완료"}</button></div></article>; })}
</div>
</div>
)}
</div>
...
<span>방문 주소</span>
<select required value={addressComplex} onChange={event => { setAddressComplex(event.currentTarget.value); setAddressDong(""); setBookingErrors(current => ({ ...current, address: "" })); }} aria-label="아파트 단지" className="address-fixed">
<option value="">아파트 단지 선택</option>
{APT_COMPLEXES.map(c => <option key={c.name} value={c.name}>{c.name}</option>)}
</select>
<div className="address-dong-ho">
<select required disabled={!addressComplex} value={addressDong} onChange={event => { setAddressDong(event.currentTarget.value); setBookingErrors(current => ({ ...current, address: "" })); }} aria-label="동">
<option value="">동 선택</option>
{selectedComplexDongs.map(dong => <option key={dong} value={dong}>{dong}</option>)}
</select>
