...
<img src="/hero-bathroom-steam-final.png" alt="열린 배수구에 강한 스팀을 분사하는 욕실 청소 장면" />
    ...
    <div className="about-left"><div className="portrait"><img className="profile-photo" src="/profile-navy.png" alt="직접 방문하는 담당자" /><div className="nameplate"><small>YOUR CLEANER</small><b>홈크린마스터</b></div></div><div className="about-copy"><h2 className="visitor-title"><span>누가 방문하는지,</span><em>미리 확인하세요.</em></h2><blockquote>“낯선 작업자가 오는 불안 없이,<br /><span className="quote-indent">사진 속 제가 직접 방문합니다.”</span></blockquote></div></div>
...
패턴에 맞춰 조율합니다.</p></div>
            ...
            <div className="calendar-days">{calendarCells.map((day, i) => day ? <button type="button" key={i} className={`${selectedDate === day ? "selected " : ""}fully-booked`} disabled={!adminMode} aria-label={`${day}일 예약 마감`} onClick={() => { setSelectedDate(day); setSelectedTimeValue(""); setSent(false); }}><span>{day}</span></button> : <i key={i} />)}</div>
            <div className="selected-booking always-visible">
              ...
              {adminMode ? <><div className="admin-blacklist"><strong>블랙리스트</strong><div className="admin-blacklist-fields"><select aria-label="블랙리스트 동 선택" value={blacklistDong} onChange={event => setBlacklistDong(event.currentTarget.value)}><option value="">동 선택</option>{Array.from({ length: 13 }, (_, i) => `${521 + i}동`).map(dong => <option key={dong} value={dong}>{dong}</option>)}</select><input aria-label="블랙리스트 호수 입력" value={blacklistHo} onChange={event => setBlacklistHo(event.currentTarget.value)} placeholder="호수 입력" /></div></div><div className="admin-slot-control"><h3>예약 시간 관리</h3><p>버튼을 눌러 예약 가능 여부를 변경하세요.</p>{[["09:00","오전 9시"],["15:00","오후 3시"],["17:00","오후 5시"]].map(([time,label]) => { const booked = (bookedSlots[selectedDateKey] ?? []).includes(time); const closed = (closedSlots[selectedDateKey] ?? []).includes(time); const booking = adminBookings.find(item => item.booking_time.slice(0, 5) === time); return <div className="admin-time-row" key={time}><button type="button" disabled={booked} className={booked || closed ? "closed" : "open"} onClick={() => toggleSlot(time)}><span>{label}</span><b>{booked || closed ? "예약 마감" : "예약 가능"}</b></button>{booking && <div className="admin-booking-info"><span><b>{booking.name}</b> · {booking.phone}<small>{booking.service}</small><small>{booking.address}</small></span><button type="button" disabled={booking.completed} onClick={() => completeBooking(booking.id)}>{booking.completed ? "서비스 완료 ✓" : "서비스 완료"}</button></div>}</div>; })}</div></> : <>
                {selectedDate && <fieldset className="time-select"><legend><small>TIME SELECT</small><strong>{calendarMonth + 1}월 {selectedDate}일 ({new Date(calendarYear, calendarMonth, selectedDate).toLocaleDateString("ko-KR", { weekday: "short" })})에 방문 가능한 시간</strong></legend><label><input checked={selectedTimeValue === "09:00"} disabled={!adminMode || (closedSlots[selectedDateKey] ?? []).includes("09:00") || (bookedSlots[selectedDateKey] ?? []).includes("09:00")} type="radio" name="booking-time" value="09:00" onChange={() => { setSelectedTimeValue("09:00"); setBookingErrors(current => ({ ...current, datetime: "" })); }} /><span>◷ 오전 9시 · 예약 마감</span><span className="time-select-action">예약 마감</span></label><label><input checked={selectedTimeValue === "15:00"} disabled={!adminMode || (closedSlots[selectedDateKey] ?? []).includes("15:00") || (bookedSlots[selectedDateKey] ?? []).includes("15:00")} type="radio" name="booking-time" value="15:00" onChange={() => { setSelectedTimeValue("15:00"); setBookingErrors(current => ({ ...current, datetime: "" })); }} /><span>◷ 오후 3시 · 예약 마감</span><span className="time-select-action">예약 마감</span></label><label><input checked={selectedTimeValue === "17:00"} disabled={!adminMode || (closedSlots[selectedDateKey] ?? []).includes("17:00") || (bookedSlots[selectedDateKey] ?? []).includes("17:00")} type="radio" name="booking-time" value="17:00" onChange={() => { setSelectedTimeValue("17:00"); setBookingErrors(current => ({ ...current, datetime: "" })); }} /><span>◷ 오후 5시 · 예약 마감</span><span className="time-select-action">예약 마감</span></label>{bookingErrors.datetime && <small className="field-error">{bookingErrors.datetime}</small>}</fieldset>}
